...
function createLocatorSortedTableHtml(locatorBookObject){
  let locatorSortedTableHtml = `<!DOCTYPE html>
  <html lang="en">
...
text-decoration:none
  ...
  const table = Object.keys(locatorBookObject).map(book =>
    locatorBookObject[book].map((data, index) => (
      `${index ===0 ? `<tr id=${book} class="book-header-row">
            <td colSpan="3">
              <h2>${book}</h2>
            </td>
          </tr>`:""}
        <tr>
          <td>
            <a href="${`https://suttacentral.net/${data[0]}/en/sujato`}" title="${getSuttaBlurb(data[0])}" target="_blank" rel="noreferrer"> 
