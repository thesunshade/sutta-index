...
let data;
try {
  const tsvFileContents = fs.readFileSync("./src/private/AN-index-Wisdom-cleanedup.txt", "utf8");
...
try {
  fs.writeFileSync("./src/private/an-array.js", output);
...
try {
  fs.writeFileSync("./src/private/an-seeAlso.csv", seeAlsoString);
...
try {
  fs.writeFileSync("./src/private/an-newArray.js", newArrayOutput);
...
try {
  fs.writeFileSync("./src/private/an-locationArray.js", locationArrayString);
...
try {
  fs.writeFileSync("./src/private/an-locationArray.csv", locationsCsv);
  console.log("new location array written");
} catch (err) {
  console.error(err);
}
